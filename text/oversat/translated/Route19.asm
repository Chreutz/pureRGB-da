```asm
_Route19CooltrainerM1BattleText::
	text "Skal varme op"
	line "før min svømmetur!"
	done

_Route19CooltrainerM1EndBattleText::
	text "Helt"
	line "varmet op!"
	prompt

_Route19CooltrainerM1AfterBattleText::
	text "Tak, knægt! Jeg er"
	line "klar til at svømme!"
	done

_Route19CooltrainerM2BattleText::
	text "Vent! Du får"
	line "hjertestop!"
	done

_Route19CooltrainerM2EndBattleText::
	text "Uuh!"
	line "Det er koldt!"
	prompt

_Route19CooltrainerM2AfterBattleText::
	text "Pas på"
	line "TENTACOOL!"
	done

_Route19Swimmer1BattleText::
	text "Jeg elsker at"
	line "svømme! Hvad med dig?"
	done

_Route19Swimmer1EndBattleText::
	text "Mave"
	line "plask!"
	prompt

_Route19Swimmer1AfterBattleText::
	text "Jeg kan slå #MON"
	line "i svømning!"
	done

_Route19Swimmer2BattleText::
	text "Hvad er der"
	line "bag horisonten?"
	done

_Route19Swimmer2EndBattleText::
	text "Glub!"
	prompt

_Route19Swimmer2AfterBattleText::
	text "Jeg ser et par"
	line "øer!"
	done

_Route19Swimmer3BattleText::
	text "Jeg prøvede at dykke"
	line "efter #MON, men"
	cont "det gik ikke!"
	done

_Route19Swimmer3EndBattleText::
	text "Hjælp!"
	prompt

_Route19Swimmer3AfterBattleText::
	text "Du skal fiske"
	line "efter hav #MON!"
	done

_Route19Swimmer4BattleText::
	text "Jeg ser på havet"
	line "for at glemme!"
	done

_Route19Swimmer4EndBattleText::
	text "Uuh!"
	line "Traumatisk!"
	prompt

_Route19Swimmer4AfterBattleText::
	text "Jeg ser på havet"
	line "for at glemme!"
	done

_Route19Swimmer5BattleText::
	text "Åh, jeg elsker"
	line "din køretur! Må jeg"
	cont "få den, hvis jeg vinder?"
	done

_Route19Swimmer5AfterBattleText::
	text "Der er stadig et"
	line "stykke vej til"
	cont "SEAFOAM ISLANDS."
	done

_Route19Swimmer6BattleText::
	text "Svømning er skønt!"
	line "Solskoldninger er det ikke!"
	done

_Route19Swimmer6EndBattleText::
	text "Chok!"
	prompt

_Route19Swimmer6AfterBattleText::
	text "Min kæreste"
	line "ville svømme til"
	cont "SEAFOAM ISLANDS."
	done

_Route19Swimmer7BattleText::
	text "Disse vande er"
	line "farlige!"
	done

_Route19Swimmer7EndBattleText::
	text "Uuh!"
	line "Farligt!"
	prompt

_Route19Swimmer7AfterBattleText::
	text "Jeg fik krampe!"
	line "Glub, glub<...>"
	done

_Route19Swimmer8BattleText::
	text "Jeg svømmede hertil,"
	line "men jeg er træt."
	done

_Route19Swimmer8EndBattleText::
	text "Jeg er"
	line "udmattet<...>"
	prompt

_Route19Swimmer8AfterBattleText::
	text "LAPRAS er så stor,"
	line "den holder dig tør"
	cont "på vandet."
	done

_Route19SignText::
	text "HAVRUTE 19"
	line "FUCHSIA CITY -"
	cont "SEAFOAM ISLANDS"
	done

```