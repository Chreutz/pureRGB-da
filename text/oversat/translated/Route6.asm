```asm
_Route6CooltrainerM1BattleText::
	text "Hvem er der?"
	line "Hold op med at"
	cont "lytte til os!"
	done

_Route6CooltrainerM1EndBattleText::
	text "Jeg"
	line "kan bare ikke vinde!"
	prompt

_Route6CooltrainerAfterBattleText::
	text "Hviske<...>"
	line "hviske<...>"
	done

_Route6CooltrainerF1BattleText::
	text "Undskyld mig! Dette"
	line "er en privat"
	cont "samtale!"
	done

_Route6CooltrainerF1EndBattleText::
	text "Ugh!"
	line "Jeg hader at tabe!"
	prompt

_Route6Youngster1BattleText::
	text "Der er ikke mange"
	line "insekter herude."
	done

_Route6Youngster1EndBattleText::
	text "Nej!"
	line "Du laver sjov!"
	prompt

_Route6Youngster1AfterBattleText::
	text "Jeg kan lide insekter,"
	line "så jeg går tilbage til"
	cont "VIRIDIAN FOREST."
	done

_Route6CooltrainerM2BattleText::
	text "Huh? Vil du"
	line "tale med mig?"
	done

_Route6CooltrainerM2EndBattleText::
	text "Jeg"
	line "startede det ikke!"
	prompt

_Route6CooltrainerM2AfterBattleText::
	text "Jeg burde have"
	line "flere #MON med"
	cont "mig for sikkerhed."
	done

_Route6CooltrainerF2BattleText::
	text "Mig? Nå, OK."
	line "Jeg spiller med!"
	done

_Route6CooltrainerF2EndBattleText::
	text "Det"
	line "virkede bare ikke!"
	prompt

_Route6CooltrainerF2AfterBattleText::
	text "Jeg vil blive"
	line "stærkere! Hvad er"
	cont "din hemmelighed?"
	done

_Route6Youngster2BattleText::
	text "Jeg har aldrig set"
	line "dig før!"
	cont "Er du god?"
	done

_Route6Youngster2EndBattleText::
	text "Du"
	line "er for god!"
	prompt

_Route6Youngster2AfterBattleText::
	text "Er mine #MON"
	line "svage? Eller er jeg"
	cont "bare dårlig?"
	done

_Route6BattleText7::
	text "Huh! Vagten"
	line "lader mig ikke"
	cont "komme igennem!"
	para "Jeg kæmper mod dig"
	line "i stedet, tror jeg!"
	done

_Route6EndBattleText7::
	text "Hvad"
	line "skal jeg gøre?"
	prompt

_Route6AfterBattleText7::
	text "Noget er i gang"
	line "i SAFFRON CITY"
	cont "hvis de har lukket"
	cont "portene."
	para "Jeg undrer mig over hvad?"
	done

_Route6BattleText8::
	text "Heheheheh<...>"
	line "den flugt var"
	cont "perfekt!"
	para "Jeg er allerede ved"
	line "SAFFRON CITY's"
	cont "udkant!"
	done

_Route6EndBattleText8::
	text "Gisp!"
	line "Jeg smutter!"
	prompt

_Route6AfterBattleText8::
	text "Ingen måde jeg"
	line "bliver fanget!"
	para "Ikke efter mit store"
	line "kup i SAFFRON"
	cont "CITY!"
	para "Alle var for"
	line "distraherede til at"
	cont "lægge mærke til mig."
	para "Stedet var i"
	line "kaos!"
	done

_Route6UndergroundPathSignText::
	text "UNDERGRUNDSSTI"
	line "CERULEAN CITY -"
	cont "VERMILION CITY"
	done

_Route6TrainerTipsText::
	text "<TIPS>"
	para "I PC'en, rul til"
	line "UDTAG, tryk så"
	cont "på SELECT."
	para "Du kan se info"
	line "om #MON i den"
	cont "nuværende boks."
	para "Dette kan gøres"
	line "selv med et fuldt"
	cont "hold!"
	done

```